...
- (id)init
{
    return [self initWithFrequency:SINE_WAVE_TONE_GENERATOR_FREQUENCY_DEFAULT];
}

- (id) initWithFrequency:(double)hertz {
    if (self = [super init]) {
        _numChannels = 1;
...
- (void)playForDuration:(NSTimeInterval)time {
    [self playThing];
    [self performSelector:@selector(stopThing) withObject:nil afterDelay:time];
}
- (void) playThing {
    if (!_toneUnit) {
